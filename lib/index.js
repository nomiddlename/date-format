...
(function (publics) {
  ...
  privates.offset = function (date) {
    ...
    var millisecond = Math.abs(date.getTimezoneOffset()),
      heure = String(Math.floor(millisecond / 60)),
    ...
    return date.getTimezoneOffset() < 0 ? "+" + heure + minute : "-" + heure + minute;
  };
  ...
  publics.utils.dateFormat = function (/*format,*/ date, monthNames, dayNames) {
    ...
    formatted = format
      ...
      .replace(/(\\y{1,4}|y{1,4})/g, function (match) { return privates.backslashNoReplace(match, year); })
